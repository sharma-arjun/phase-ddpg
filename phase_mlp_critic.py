...
class PMLP(nn.Module):
	def __init__(self, input_size_state, input_size_action, output_size, hidden_size, dtype, n_layers=1, batch_size=1, scale=1.0, tanh_flag=0):
		...
		self.l_00 = nn.Linear(self.input_size_state, self.hidden_size_1).type(dtype)
		self.h2o_0 = nn.Linear(self.hidden_size_2, self.output_size).type(dtype)
		#self.gru_10 = nn.GRUCell(self.input_size, self.hidden_size)
		self.l_10 = nn.Linear(self.input_size_state, self.hidden_size_1).type(dtype)
		self.h2o_1 = nn.Linear(self.hidden_size_2, self.output_size).type(dtype)
		#self.gru_20 = nn.GRUCell(self.input_size, self.hidden_size)
		self.l_20 = nn.Linear(self.input_size_state, self.hidden_size_1).type(dtype)
		self.h2o_2 = nn.Linear(self.hidden_size_2, self.output_size).type(dtype)
		#self.gru_30 = nn.GRUCell(self.input_size, self.hidden_size)
		self.l_30 = nn.Linear(self.input_size_state, self.hidden_size_1).type(dtype)
		self.h2o_3 = nn.Linear(self.hidden_size_2, self.output_size).type(dtype)
		...
		dummy_x = Variable(torch.zeros(batch_size, input_size_state+input_size_action), requires_grad=False).type(dtype)
		...
		if n_layers == 1:
			for l, h2o in zip(self.control_hidden_list[0], self.control_h2o_list):
                                dummy_x_s = dummy_x[:,0:input_size_state]
                                dummy_x_a = dummy_x[:,input_size_state:]
				dummy_h = F.relu(l(dummy_x_s))
				dummy_o = h2o(dummy_h)
		...
		if n_layers == 2:
			for l0, l1, h2o in zip(self.control_hidden_list[0], self.control_hidden_list[1], self.control_h2o_list):
                                dummy_x_s = dummy_x[:,0:input_size_state]
                                dummy_x_a = dummy_x[:,input_size_state:]
				dummy_h0 = F.relu(l0(dummy_x_s))
				dummy_h1 = l1(torch.cat((dummy_h0,dummy_x_a),1))
	...
	def forward(self,x,phase):
		...
		#hiddens = []
		#l0 = nn.Linear(self.input_size_state, self.hidden_size_1).type(self.dtype)
		#hiddens.append(l0)
		#h2o = nn.Linear(self.hidden_size_2, self.output_size).type(self.dtype)
		#if self.n_layers == 2:
		#	l1 = nn.Linear(self.hidden_size_1+self.input_size_action, self.hidden_size_2).type(self.dtype)
		#	hiddens.append(l1)
		...
		#h_0 = F.relu(l0(x_s))
		#if self.n_layers == 2:
		#	h_1 = F.relu(l1(torch.cat((h_0,x_a),1)))
		#	if self.tanh_flag:
		#		o = F.tanh(h2o(h_1))
		#	else:
		#		o = h2o(h_1)

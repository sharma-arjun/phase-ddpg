...
class PMLP(nn.Module):
	def __init__(self, input_size, output_size, hidden_size, dtype, n_layers=1, batch_size=1, scale=1.0, tanh_flag=0):
		...
		self.l_00 = nn.Linear(self.input_size, self.hidden_size_1).type(dtype)
		...
		self.l_10 = nn.Linear(self.input_size, self.hidden_size_1).type(dtype)
		...
		self.l_20 = nn.Linear(self.input_size, self.hidden_size_1).type(dtype)
		...
		self.l_30 = nn.Linear(self.input_size, self.hidden_size_1).type(dtype)
	...
	def forward(self,x,phase):
		...
		#hiddens = []
		#l0 = nn.Linear(self.input_size, self.hidden_size_1).type(self.dtype)
		#hiddens.append(l0)
		#h2o = nn.Linear(self.hidden_size_2, self.output_size).type(self.dtype)
		#if self.n_layers == 2:
		#	l1 = nn.Linear(self.hidden_size_1, self.hidden_size_2).type(self.dtype)
		#	hiddens.append(l1)
